...
def test_tpu_checkpointing_single_host(ray_tpu_single_host, tmp_path):
    ...
    def train_func_checkpointing():
        ...
        for step, loss in enumerate(losses):
            ...
            # Step 2: all 2s
            with mesh:
                w = jax.device_put(jnp.ones((8, 8)) * step, sharding) 
        ...
        # Check if best step exists and others are deleted (assuming orbax handles cleanup immediately or we check correctly)
        # Note: Orbax/Ray might keep them around until delete triggers? 
        # JaxCheckpointManager uses Ray's CheckpointConfig which maps to Orbax options.
        ...
        restore_target = {"w": jax.make_array_from_callback(
            (8, 8), sharding, lambda idx: np.zeros([(s.stop if s.stop is not None else 8) - (s.start if s.start is not None else 0) for s in idx]))}
                
        restored = manager.restore(restore_target, best_step)
        
        # Verify restored value matches step 1 (all 1s)
        with mesh:
             expected_w = jax.device_put(jnp.ones((8, 8)) * best_step, sharding)
...
def test_tpu_checkpointing_multi_host(ray_tpu_multi_host, tmp_path):
    ...
    def train_func_checkpointing():
        ...
        with mesh:
            ...
            w_save = jax.device_put(w, sharding_save)
        ...
        restored = manager.restore(restore_target, 0)

        # Verify values are correct
        is_equal = bool(jnp.array_equal(restored["w"], w))
    ...
    trainer = JaxTrainer(
        ...
        run_config=RunConfig(
            ...
            callbacks=[CustomMetricsCallback(actor_name)],
            worker_runtime_env={
                "env_vars": {"JAX_PLATFORMS": "cpu", "XLA_FLAGS": "--xla_force_host_platform_device_count=4"}
            },
        ),
    )
