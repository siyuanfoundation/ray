...
The node selection process will evaluate candidate worker groups for scale-up using a strict selection hierarchy:
1. **Utilization**: Maximize resource utilization first (current behavior).
2. **Recoverable Availability**: Prefer nodes that are currently available or have recovered from failures.
3. **Priority**: Prefer higher-priority nodes when utilization and availability are equal.
4. **Failure Recency**: Use historical failure recency as the final tie-breaker.
...
2.  **Data Propagation**: Update `SchedulingNode` in `python/ray/autoscaler/v2/scheduler.py` to include and carry the `priority` and availability scores.
