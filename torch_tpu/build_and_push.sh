...
docker build --build-arg BUILD_DATE="$BUILD_DATE" --build-arg AUTH_TOKEN="$(gcloud auth print-access-token)" --build-arg GITHUB_USER="${GITHUB_USER}" --build-arg GITHUB_TOKEN="${GITHUB_TOKEN}" -t "${IMAGE_PATH}" -f Dockerfile ..
